...
from multildap_bvt_preconfigs import MultildapBVTConfigs
...
from component_pages.navigator.nav_legacy import NavigatorLegacyRoute
from component_pages.ier.ier import IERDownloader
from component_pages.ier.ier_nav import IER_NavigatorAutomation
...
def main():
    ...
    #If the deployment is multildap , perform configurations for the second ldap user.
    try:
        multildapconf= MultildapBVTConfigs()
        if multildap.lower() == "true":
            multildapconf.main()
    except Exception as e:
        logger.error("Multildap Configurations failed...")
        raise RuntimeError("Critical error: Multildap Configurations failed... Exiting...")
    ...
    try :
        ...
        cpe_admin = CpeAdmin()
        cpe_admin.login()
        mail_services_status =  cpe_admin.enable_mail_services()
        cpe_admin.navigate_to_object_store()
        cpe_admin.verify_and_capture_object_store()
        cpe_admin.navigate_to_os01_index_area() #OS01 Index area
    ...
    if "IER" in optional_components:
        ...
        try:
            #IER_Navigator
            ier_nav = IER_NavigatorAutomation()
            e = ier_nav.login_to_navigator()
            if e == 1 :
                ier_nav.navigate_to_administration()
            else :
                pass
        except Exception as e:
            handle_exception("IER_Plugin", e)
        finally:
                ier_nav.close_navigator()
    else : 
    ...
    if check_cr_content.check_cr_content("navigator:") :
        try :
            ...
            if mail_services_status == "PASSED" :
                nav_utility.send_mail()
            nav_utility.about()
        except Exception as e:
        ...
        try :
            ...
            nav_cmod.navigate_to_administration()
            nav_cmod.CMOD_repo()
            nav_cmod.CMOD_desktop()
        ...
        try :
            ...
            nav_cm8.navigate_to_administration()
            nav_cm8.CM8_repo()
            nav_cm8.CM8_desktop()
            nav_cm8.capture_nav1()
        ...
        try : 
            #Navigator Legacy Route
            nav_legacy = NavigatorLegacyRoute()
            nav_legacy.login_to_navigator()
            nav_legacy.navigate_to_second_link()
        except Exception as e:
            handle_exception("Navigator", e)
        finally :
            nav_legacy.close_navigator()
    else : 
    ...
    try:
        ...
        logger.error("Error while executing fisma bvt")
    ...
    try:
        ...
        handle_exception("Checking CR", e)
    ...
    try:
        ...
        json_to_xml.generate_xml_report("bvt_admin_xml_report")
    except Exception as e:
        handle_exception("Report Generation", e)
        
    #MVT 
    try : 
        contentMVT.mvt_runner("CP4BA", True)
    except Exception as e:
        handle_exception("MVT", e)
