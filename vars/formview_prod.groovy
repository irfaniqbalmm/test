            ...
            steps {
		        withCredentials([usernamePassword(credentialsId: 'github-credentials', usernameVariable: 'GITHUB_USER', passwordVariable: 'GITHUB_PASS')]) {
                    script {
                        ...
                        sh('curl -u ${GITHUB_USER}:${GITHUB_PASS} -o /tmp/clusters_data.json https://raw.github.ibm.com/dkrishan/Cp4ba-Automation/formview_prod/CP4BA_Package/clusters.json')
            ...
            script {
                ...
                sh """
                    sshpass -p '${ROOT_PASSWORD}' ssh -o StrictHostKeyChecking=no root@${env.SSH_URL} '
                        ...
                        echo "@@ DB: ${dbname}" && \
                        echo "@@ LDAP: ${ldapname}" && \
                        echo "@@ Fips: ${params.FIPS}" && \
                        echo "@@ Egress: ${params.EGRESS}" && \
                        ocpversion=\$(oc version | grep Server) && \
                        echo "@@ \${ocpversion}"  && \
                        echo "@@ Deployment type: ${params.STAGE_OR_PROD}" && \
                        ...
                        # ---- Install Playwright system dependencies (RHEL/CentOS/Rocky/Alma) ----
                        yum install -y \\
                            atk \\
                            at-spi2-atk \\
                            alsa-lib \\
                        ...
                        echo "Installation of system + python + playwright packages done."
                    '
                """
                ...
                withCredentials([usernamePassword(credentialsId: 'github-credentials', usernameVariable: 'GITHUB_USER', passwordVariable: 'GITHUB_PASS'), string(credentialsId: 'entitlement_key', variable: 'entitlement_key'), string(credentialsId: 'entitlement_prod', variable: 'entitlement_prod'), string(credentialsId: 'pull_secret_token', variable: 'pull_secret_token')]) {
                    script{
                        ...
                        sh """
                            ...
                            git clone -b formview_prod --depth 1 --recurse-submodules --shallow-submodules https://${GITHUB_USER}:${GITHUB_PASS}@github.ibm.com/dkrishan/Cp4ba-Automation.git && \
            ...
            steps{
                script{
                    ...
                    def scriptstatus = sh(script: """
                        ...
                        cd /opt/Cp4ba-Automation/cp4ba_proddeploy_automation && \
                        PYTHONPATH=/opt/Cp4ba-Automation \
                        python3 -m formview_prod.formview_prod \"${escapedJsonStr}\"
                        deactivate
                        '
                    """, returnStatus: true)
                    ...
                    echo "${new Date()} :- CR created. Waiting for 2.5 hour before proceeding...."
                    sleep(time: 150, unit: 'MINUTES')
