...
pipeline {
    ...
    stages {
                ...
                script {
                    echo "Installing the required  python packages "
                     sh """
                        ...
                        echo "@@ Fisma: ${params.FISMA.toUpperCase()} " && \
        ...
        stage('Custom Host & Certs'){
            steps{
                script{
                    echo """Parameters being passed to custom_resource.py:
                        Project Name: ${params.PROJECT_NAME}
                       
                    """
                    def hostConfig = ["project": new_pjt_name, "cluster_pass": KUBE_PASS, "cluster_name": cluster]
                    def jsonhostConfig = JsonOutput.toJson(hostConfig)         
                    def escapedhostConfig = jsonhostConfig.replace('"', '\\"')
                    echo "Passing JSON: ${jsonhostConfig}"
                    echo """ ${jsonhostConfig} """

                    def customHostStatus = sh(script: """
                        sshpass -p '${ROOT_PASSWORD}' ssh -o StrictHostKeyChecking=no root@${env.SSH_URL} ' \
                        oc login https://api.${cluster}.cp.fyre.ibm.com:6443 -u kubeadmin -p ${KUBE_PASS} --insecure-skip-tls-verify && \
                        ...
                        source /opt/automationenv/bin/activate && \
                        cd /opt/Cp4ba-Automation/cp4ba_proddeploy_automation && \
                        python3 ./custom_resource.py \"${escapedhostConfig}\"
                        deactivate
                        '
                    """, returnStatus: true)

                    // Print the result to the Jenkins log
                    echo "Python script output: ${customHostStatus}"

                    if (customHostStatus != 0) {
                        error "Custom host and certificate enablement failed with exit code ${status}"
                    }
                    echo """Custom host and certificate ${params.PROJECT_NAME} deploying on ${cluster}"""
        ...
        stage('Network Policy operations') {
            when {
                expression {
                ...
                withCredentials([usernamePassword(credentialsId: 'github-credentials', usernameVariable: 'GITHUB_USER', passwordVariable: 'GITHUB_PASS')]) {
                    script {
                        sh """
                            ...
                            '
                        """
                        echo "Network configuration is installed"
                    }
                }
            }
        }
